...
public interface IProductRepository extends JpaRepository<Product, Long> {
    @Query(nativeQuery = true, value = "select product.id as idProduct,\n" +
            ...
            "         join language l on l.id = product_language.language_id where name_language ='tiếng việt' and product_language.name='kala'")
    GetProduct getByNameKaLaVi();
    ...
    @Query(nativeQuery = true, value = "select product.id as idProduct,\n" +
            ...
            "from product_language\n" +
            "         join product p on product_language.product_id = p.id\n" +
            "         join language l on l.id = product_language.language_id where name_language ='tiếng việt' and product_language.name='Rắn naga 5 đầu'")
    GetProduct getByNameNagaVi();
    @Query(nativeQuery = true, value = "select img as imgProduct,\n" +
            ...
            "from product_language\n" +
            "         join product p on product_language.product_id = p.id\n" +
            "         join language l on l.id = product_language.language_id where name_language ='tiếng champ' and product_language.name='ul% ng% 5 a-k<K'")
